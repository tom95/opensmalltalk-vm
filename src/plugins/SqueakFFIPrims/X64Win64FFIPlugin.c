/* Automatically generated by
	VMPluginCodeGenerator VMMaker.oscogSPC-eem.2112 uuid: 2cb05138-53ee-44f4-9ef2-687f081b72d6
   from
	ThreadedX64Win64FFIPlugin VMMaker.oscogSPC-eem.2112 uuid: 2cb05138-53ee-44f4-9ef2-687f081b72d6
 */
static char __buildInfo[] = "ThreadedX64Win64FFIPlugin VMMaker.oscogSPC-eem.2112 uuid: 2cb05138-53ee-44f4-9ef2-687f081b72d6 " __DATE__ ;
...
static sqInt ffiReturnStructofTypein(usqLong intRet, sqInt ffiRetType, CalloutState *calloutState);
...
#ifdef SQUEAK_BUILTIN_PLUGIN
	"X64Win64FFIPlugin VMMaker.oscogSPC-eem.2112 (i)"
#else
	"X64Win64FFIPlugin VMMaker.oscogSPC-eem.2112 (e)"
#endif
;
...
#define dispatchFunctionPointerwithwithwithwithwithwith(aFunctionPointer,a1,a2, a3, a4,a5,a6) (aFunctionPointer)(a1,a2, a3, a4,a5,a6)
...
l3:	/* end ffiFloatValueOf: */;
	...
	if (atomicType == FFITypeSingleFloat) {
		...
		if (((calloutState->floatRegisterIndex)) < NumFloatRegArgs) {
			...
			(calloutState->floatRegisterSignature) = ((calloutState->floatRegisterSignature)) + (1U << ((calloutState->floatRegisterIndex)));
			(calloutState->integerRegisterIndex) = ((calloutState->floatRegisterIndex) = ((calloutState->floatRegisterIndex)) + 1);
		}
		else {
			...
			if ((structSize <= WordSize)
			 && ((structSize & (structSize - 1)) == 0)) {
				...
				if (((calloutState->integerRegisterIndex)) < NumIntRegArgs) {
					...
					(calloutState->floatRegisterIndex) = ((calloutState->integerRegisterIndex) = ((calloutState->integerRegisterIndex)) + 1);
				}
				else {
					if ((((calloutState->currentArg)) + WordSize) > ((calloutState->limit))) {
						return FFIErrorCallFrameTooBig;
...
#        endif /* COGMTVM */
				...
				if ((structSize1 <= WordSize)
				 && ((structSize1 & (structSize1 - 1)) == 0)) {
					memcpy((&arg1), ptrAddress, structSize1);
					/* begin ffiPushUnsignedLongLong:in: */
					if (((calloutState->integerRegisterIndex)) < NumIntRegArgs) {
						...
						longAtput((calloutState->currentArg), arg1);
						(calloutState->currentArg = ((calloutState->currentArg)) + WordSize);
					}
					return 0;
				}
 ...
 */
	...
	/* ThreadedX64Win64FFIPlugin>>#ffiCalloutTo:SpecOnStack:in: */
static sqInt
ffiCalloutToSpecOnStackin(void *procAddr, sqInt specOnStack, CalloutState *calloutState)
{
	...
	myThreadIndex = 0;
	/* begin maybeDisownVM:threadIndexInto: */
	
#  if COGMTVM
	if (((calloutState->callFlags)) & FFICallFlagThreaded) {
		threadIndex = disownVM(0);
...
#  endif /* COGMTVM */
	...
	if (((calloutState->floatRegisterSignature)) > 0) {
		loadFloatRegs(((calloutState->floatRegisters))[0], ((calloutState->floatRegisters))[1], ((calloutState->floatRegisters))[2], ((calloutState->floatRegisters))[3], ((calloutState->floatRegisters))[4]);
	}
...
#endif /* SPURVM */
;
		memcpy(firstIndexableField(oop1), (returnStructInRegisters((calloutState->structReturnSize))
			? (&intRet)
...
ffiLogCallsTo(char*fileName)
{
	...
	/* ThreadedX64FFIPlugin>>#ffiPushDoubleFloat:in: */
static sqInt
...
ffiPushSignedShortin(sqInt value, CalloutState *calloutState)
{
	...
	/* ThreadedX64FFIPlugin>>#ffiPushSingleFloat:in: */
static sqInt
ffiPushSingleFloatin(float value, CalloutState *calloutState)
{
	if (((calloutState->floatRegisterIndex)) < NumFloatRegArgs) {
		(((float *) (((calloutState->floatRegisters)) + ((calloutState->floatRegisterIndex)))))[0] = value;
		/* begin incrementFloatRegisterIndex */
		(calloutState->floatRegisterSignature) = ((calloutState->floatRegisterSignature)) + (1U << ((calloutState->floatRegisterIndex)));
		(calloutState->integerRegisterIndex) = ((calloutState->floatRegisterIndex) = ((calloutState->floatRegisterIndex)) + 1);
	}
...
#      endif /* COGMTVM */
	...
	/* ThreadedX64Win64FFIPlugin>>#ffiPushStructure:ofSize:typeSpec:ofLength:in: */
static sqInt
...
/*	Create a structure return value from an external function call. The value
	...
	/* ThreadedX64Win64FFIPlugin>>#ffiReturnStruct:ofType:in: */
static sqInt
...
#endif /* SPURVM */
;
	memcpy(firstIndexableField(oop), (returnStructInRegisters((calloutState->structReturnSize))
		? (&intRet)
		: (calloutState->limit)), (calloutState->structReturnSize));
	storePointerofObjectwithValue(0, retOop, oop);
	return retOop;
...
/*	There are only 4 args passed by register int or float.
	...
	Consequently, floatRegisterIndex cannot be used anymore to detect presence
	of float parameter.
	However, we set a signature bitmap indicating which register position is
	used to pass a float.
	
	IMPLEMENTATION NOTES:
	There are code generator hacks that bypass the accessors.
	So we cannot just redefine the method floatRegisterIndex as
	^integerRegisterIndex. Instead we must maintain the two indices
 */
...
{
	(self_in_incrementFloatRegisterIndex->floatRegisterSignature) = ((self_in_incrementFloatRegisterIndex->floatRegisterSignature)) + (1U << ((self_in_incrementFloatRegisterIndex->floatRegisterIndex)));
	return ((self_in_incrementFloatRegisterIndex->integerRegisterIndex) = ((self_in_incrementFloatRegisterIndex->floatRegisterIndex) = ((self_in_incrementFloatRegisterIndex->floatRegisterIndex)) + 1));
}
...
{
	return ((self_in_incrementIntegerRegisterIndex->floatRegisterIndex) = ((self_in_incrementIntegerRegisterIndex->integerRegisterIndex) = ((self_in_incrementIntegerRegisterIndex->integerRegisterIndex)) + 1));
}
	...
	if ((((calloutState1->structReturnSize)) > 0)
	 ...
	 && (!(returnStructInRegisters((calloutState1->structReturnSize)))))) {
		...
		if (((calloutState1->integerRegisterIndex)) < NumIntRegArgs) {
			...
			(calloutState1->floatRegisterIndex) = ((calloutState1->integerRegisterIndex) = ((calloutState1->integerRegisterIndex)) + 1);
		}
		else {
...
#  if COGMTVM
	if (((calloutState1->callFlags)) & FFICallFlagThreaded) {
		threadIndex1 = disownVM(0);
		myThreadIndex1 = threadIndex1;

	}

#  endif /* COGMTVM */

	if (((calloutState1->floatRegisterSignature)) > 0) {
		loadFloatRegs(((calloutState1->floatRegisters))[0], ((calloutState1->floatRegisters))[1], ((calloutState1->floatRegisters))[2], ((calloutState1->floatRegisters))[3], ((calloutState1->floatRegisters))[4]);
	}
	if ((allocaLiesSoSetSpBeforeCall())
	 || (mustAlignStack())) {
...
#endif /* SPURVM */
;
		memcpy(firstIndexableField(oop11), (returnStructInRegisters((calloutState1->structReturnSize))
			? (&intRet1)
...
/*	Answer if a struct result of a given size is returned in memory or not. */
	...
	/* ThreadedX64Win64FFIPlugin>>#returnStructInRegisters: */
static sqInt
returnStructInRegisters(sqInt returnStructSize)
{
	return (returnStructSize <= WordSize)
	 && ((returnStructSize & (returnStructSize - 1)) == 0);
}
